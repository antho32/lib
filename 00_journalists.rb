...
def hendle(journaliste)
    puts "Combien y a-t-il de handle dans cette array ?"
    puts "Il y a #{journaliste.length} handle dans cette array"
    puts
end
hendle(journaliste)

def motscourt(journaliste)
    puts "Quel est le handle le plus court de cette liste ?"
    puts "Le mots le plus court et #{journaliste.sort_by(&:length)[0]}"
    puts
end 
motscourt(journaliste)

#def caractere(journaliste)
#    puts"Combien y-a-t'il de handle contenant 5 caractères (le @ ne compte pas pour un caractère)"
#    puts "Il y a #{} contenant 5 caractere"
#end 
#caractere(journaliste)

def majuscule(journaliste)
    count = 0
    journaliste.each do |email|
      if email.match(/@[A-Z]/)
        count += 1
        end
      end
  puts "Combien commencent par une majuscule (première lettre juste après le @) ?"
  puts "Il y en a #{count} mots commencent par une majuscule"
  puts
end
majuscule(journaliste)


def sort(journaliste)
...
def epenser(journaliste)
    puts "Quelle est la position dans l'array de la personne @epenser ?"
    puts "@epenser est en position #{journaliste.index("@epenser")}"
    puts
end
epenser(journaliste)
